...
def make_boxplot(data, setname, graph_set_id, ylabel, savedir=None, filename_suffix=None):
	...
	args:
		data : a list lof lists that contains the data to be plotted. each sublist will induce a single boxplot
	...
	'''
	### TO DO: make data a dictionary that maps algorithm-codes to lists of results.
	###	This way, we do not have to construct the labels in an extra step
...
def make_boxplots_all(setname, axis="OUTPUT", type="ABSOLUTE"):
	...
	if type == "ABSOLUTE":
		...
		for file in files:
			...
			if algo not in data_dict:
				data_dict[algo] = []
			data_dict[algo] += load_axis_data_from_file(filepath, axis)
	...
	make_boxplot(data, setname, '', ylabel=axis+" ("+type+")", savedir=outputdir, filename_suffix='total')
